...
/**
 * Secção de comentários inline, embutida diretamente no card do post
 * (não é um pop-up/modal) — expande/colapsa por baixo da legenda, como
 * é hábito nos feeds sociais.
 */
export function CommentsSection({ open, postId, postAuthorUsername, seedComments, onCommentAdded }: CommentsSectionProps) {
  const { user } = useAuth();
    ...
    <AnimatePresence initial={false}>
      {open && (
        <motion.div
          ...
          className="overflow-hidden -mx-3 sm:-mx-4"
        >
          <div className="border-t border-border mt-2">
            <div className="max-h-72 overflow-y-auto flex flex-col gap-4 p-3 sm:p-4">
            ...
            {/* Compose */}
            <div className="flex items-center gap-3 px-3 sm:px-4 py-3 border-t border-border">
              <Avatar className="w-8 h-8 shrink-0">
